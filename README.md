# 10 multiviews(1024*1024) for each scene
# sample_count: 128
...
def render_view(spectrum, azim, elev, filename):
    ...
    if spectrum != 'nir':
        camera_pose = make_camera_pose(scene, azim, elev, 300)
        sensor_dict = {
            "type": "perspective",
        ...
        camera_pose = make_camera_pose(scene, azim, elev, 280)
        params = mi.traverse(scene)

        params['elm__2.to_world'] = camera_pose
        params["elm__2.film.size"] = [1024, 1024]
        params["elm__2.film.crop_size"] = [1024, 1024]
        params.update()
